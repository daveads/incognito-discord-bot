...
class Enable(commands.Cog):
     ...
     async def enable(self, ctx: discord.ApplicationContext):
        ...
        if disable_roleObj in ctx.user.roles:
            ...
            await ctx.respond("**Anonymous Message** `Enable`", ephemeral=True, delete_after=5)

            
        else:
            await ctx.respond("**Anonymous Message** `Already Enabled`", ephemeral=True, delete_after=5)
